...
$(document).ready(function() {
  ...
  document.onkeyup = function(data) {
    if (data.which == 27) {
      $.post("http://vrp_inventario_carro/invClose", JSON.stringify({}), function(datab) {});
    }
  };
...
var requestAjax = (option, itemKey, amount) => {
  $.post(
    "http://vrp_inventario_carro/" + option,
    JSON.stringify({
      item: itemKey,
      amount
    })
...
const updateMochila = () => {
  ...
  $.post("http://vrp_inventario_carro/requestMochila", JSON.stringify({}), data => {
    ...
    $('.bag-workspace > .inventory-title').html(`
      Inventário <small>(<i><b>${data.peso.toFixed(2)}/${data.maxpeso.toFixed(2)}KG</b>  | <b>${(data.maxpeso - data.peso).toFixed(2)}kg</b> livre</i>)</small>
    `);
    $('.trunk-workspace > .inventory-title').html(`
      Baú do carro <small>(<i><b>${data.peso2.toFixed(2)}/${data.maxpeso2.toFixed(2)}KG</b> em uso - <b>${(data.maxpeso2 - data.peso2).toFixed(2)}kg </b> livre</b></i>)</small>
    `);
...
$(document).ready(function() {
  ...
  $(document).on('click', '.amount-option button', function() {
    ...
    if(event == 'send') {
      ...
      $.post(
        "http://vrp_inventario_carro/" + paramUrl,
        JSON.stringify({
