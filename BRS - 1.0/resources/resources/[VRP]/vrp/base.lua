...
function vRP.log(file,info)
  file = io.open(file, "a")
  if file then
    file:write(os.date("%d/%m/%Y %X").." | "..info.."\r")
  end
  file:close()
end
...
function vRP.dropPlayer(source)
	...
	if user_id then
		...
		vRP.log("nzk_logs/player/desconectou.txt","O [ID: "..user_id.."] Saiu da cidade.")
	end
end
...
AddEventHandler("queue:playerConnecting",function(source,ids,name,setKickReason,deferrals)
	...
	if ids ~= nil and #ids > 0 then
		deferrals.update("[BRS] Checando Identificadores (Contas Steam e Rockstar)...")
		local user_id = vRP.getUserIdByIdentifiers(ids)
		if user_id then
			deferrals.update("[BRS] Checando Banimento...")
			if not vRP.isBanned(user_id) then
				deferrals.update("[BRS] Checando Whitelist...")
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						deferrals.update("[BRS] Carregando Dados...")
						...
						vRP.log("nzk_logs/player/entrou.txt","O "..name.." [ID: "..user_id.."] Entrou na cidade.")
						deferrals.done()
					else
						local tmpdata = vRP.getUserTmpTable(user_id)
						tmpdata.spawns = 0
					...
					deferrals.done("[BRS] Para entrar, acesse nosso Discord: discord.gg/brsrp e solicite liberação do seu ID: "..user_id.."")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("[BRS] Banido (ID: "..user_id.."). ")
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
			deferrals.done("[BRS] Ocorreu um problema de identificação.")
			TriggerEvent("queue:playerConnectingRemoveQueues",ids)
		end
	else
		deferrals.done("[BRS] Ocorreu um problema de identidade.")
		TriggerEvent("queue:playerConnectingRemoveQueues",ids)
	end
end)
