...
function tvRP.toggleCapuz()
	...
	if capuz then
		...
		SetPedComponentVariation(GetPlayerPed(-1), 1, 120,0,2)
		tvRP.removeDiv("capuz")
	end
end

function tvRP.setarRoupasPrisional()
...
local checkNight = 0
function tvRP.nightvision()
  if checkNight == 0 then
    SetPedPropIndex(GetPlayerPed(-1), 0, 116, 0,0)
    SetNightvision(true)
    checkNight = 1
  else
