...
function vRP.tryPayment(user_id,amount)
  if amount < 0 then
		vRP.log("nzk_logs/anticheat/inject.txt","**tryPayment** O [ID:"..user_id.."] Tentou Injetar Dinheiro Negativo = "..amount.."")
		return false
		else
...
function vRP.tryTransfer(user_id,amount)
	local money = vRP.getBankMoney(user_id)
	if amount >= 0 and money >= amount then
		vRP.setBankMoney(user_id,money-amount)
		return true
...
function vRP.tryFullPayment(user_id,amount)
	if amount == nil then return false
  elseif amount < 0 then
    vRP.log("nzk_logs/anticheat/inject.txt","**tryFullPayment** O [ID:"..user_id.."] Tentou Injetar Dinheiro Negativo = "..amount.."")
    return false
  else
	  local money = vRP.getMoney(user_id)
	  if money >= amount then
...
AddEventHandler("vRP:playerJoin",function(user_id,source,name)
	vRP.execute("vRP/money_init_user",{ user_id = user_id, wallet = 15000, bank = 35000 })
	local tmp = vRP.getUserTmpTable(user_id)
	if tmp then
