...
AddEventHandler("vRP:playerJoinGroup", function(user_id, group, gtype)
	...
	if player then
		if gtype == "org" then 
			connectedPlayers[user_id].job = group or ""
...
function AddPlayerToScoreboard(source, update)
	...
	if playerId then
		...
		connectedPlayers[playerId].ping = math.random(15,50)
		connectedPlayers[playerId].id = playerId
		connectedPlayers[playerId].name = GetPlayerName(source)
		connectedPlayers[playerId].job = vRP.getUserGroupByType(playerId,"org") or 0
		if update then
			TriggerClientEvent('vrp_scoreboard:updateConnectedPlayers', -1, connectedPlayers)
		end
	end
