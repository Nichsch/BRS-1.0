...
Config.Priority = {
    ["steam:11000011ced269a"] = 80, --Junnnnqueira
    ["steam:11000010ac43eb5"] = 100, -- KW  -- steam:11000010d085fc6
    ["steam:11000010ac43eb5"] = 100, -- mito tototoso
    ["steam:11000013e0fceee"] = 10, -- vip
    ["license:585c9fc93b2631f6ebf7abd210e5b57e54af4911"] = 100, -- CONE
    ["license:a3e24859fa41a01f85f95a9f573d6bf4d9cef72a"] = 100, -- VITAO
	


-- Doadores
	["steam:11000011a58d297"] = 10, -- Zenistu
	["steam:11000011d659056"] = 100, -- Nisch
	["steam:11000012c106eae"] = 20, -- Pedrão
}
...
Config.Language = {
    ...
    err = "[BRS] Erro: Não foi possível receber seus ID's de conexão, tente reconectar.",
    _err = "[BRS] Aconteceu um Erro.",
    pos = "[BRS] Você é %d/%d na Fila.",
    connectingerr = "[BRS] Aconteceu um erro ao adicionar você na lista de conexões.",
    banned = "You are banned, you may appeal it at www.whatever.net | Reason: %s",
    steam = "[BRS] Erro: Obrigatório uso da Steam.",
...
Citizen.CreateThread(function()
    local function playerConnect(name, setKickReason, deferrals)
        ...
        local function done(msg)
            ...
            -- old: if not msg then deferrals.done() else deferrals.done(tostring(msg) and tostring(msg) or "") CancelEvent() end
			--[[if not msg then
				--deferrals.done()
			else
				deferrals.done(tostring(msg) and tostring(msg) or "")
        ...
        local steamIDUser = tostring(Queue:GetSteamId(src))
        for k,v in pairs(wlList) do
            if string.match(v, steamIDUser) then
                steamNotWhitelisted = true
            end
        end

        if not steamNotWhitelisted then
            done("[BRS] Sua Steam não está autorizada: " .. steamIDUser)
            deferrals.done("[BRS] Sua Steam não está autorizada: " .. steamIDUser)
        ...
        if Queue:NotFull(true) then
			...
			Queue:DebugPrint(name .. "[" .. ids[1] .. "] is loading into the server [1]")
			TriggerEvent("queue:playerConnecting", src, ids, name, setKickReason, deferrals)
        ...
        Citizen.CreateThread(function()
            ...
            while true do
                ...
                if pos <= 1 and Queue:NotFull() then
                    ...
                    Queue:RemoveFromConnecting(ids)
                    Queue.ThreadCount = Queue.ThreadCount - 1
                    Queue:DebugPrint(name .. "[" .. ids[1] .. "] is loading into the server [2]")
    ...
    local function checkTimeOuts()
        ...
        while i <= Queue:GetSize() do
			...
			elseif (data.timeout >= 120) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                ...
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] was removed from the queue because they timed out [1]")
            else
                i = i + 1
            end
        end
        ...
        while i <= Queue:ConnectingSize() do
			...
			if ((data.timeout >= 300 and lastMsg >= 35000) or data.timeout >= 340) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                Queue:RemoveFromQueue(data.source, true)
                Queue:RemoveFromConnecting(data.source, true)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] was removed from the connecting queue because they timed out [2]")
            else
                i = i + 1
            end
        end
