	...
	if user_id then
		...
		local tattoshop_choice = function(player,choice)
			...
			if tattoo then
				...
				if tattoo == "CLEAR" then
					...
					if ok then
						TriggerClientEvent('chatMessage', source, "^2Limpeza concluida.")
					...
					if not applied then
						TriggerClientEvent('chatMessage', source, "^2Tatuagem adicionada.")
						TSclient.addTattoo(source,tattoo,shop)
						TriggerClientEvent("reloadtattos",source)
					else
						TriggerClientEvent('chatMessage', source, "^2Tatuagem removida.")
		...
		menudata.onclose = function(player)
			...
			if vRP.tryPayment(user_id,price) then
				...
				if price > 0 then
					TriggerClientEvent('chatMessage', source, "^5Pagou $"..price.." dólares.")
				end
			else
