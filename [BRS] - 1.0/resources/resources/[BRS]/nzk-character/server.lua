...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("nzk-character:characterCreate",source)
	end
end
 
RegisterServerEvent("nzk-character:finishedCharacter")
AddEventHandler("nzk-character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("nzk-character:normalSpawn",source,firstspawn)
	TriggerEvent("nzk-barbershop:init",user_id)
end