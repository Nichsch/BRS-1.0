	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Movimente com ~y~W, A, S, D~s~ e confirma com ~y~ESPAÇO~s~ para o bloco da esquerda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Use as ~y~SETAS~s~ e ~y~ENTER~s~ para o bloco da direita")
			else
