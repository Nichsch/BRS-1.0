TokoVoipConfig = {
	...
	plugin_data = {
		...
		TSChannel = "Cidade",
		TSPassword = "brsrp1", -- TeamSpeak channel password (can be empty)
		...
		TSChannelWait = "Bem-Vindo",

		-- Blocking screen informations
		TSServer = "147.135.22.103:9500", -- TeamSpeak server address to be displayed on blocking screen
		TSChannelSupport = "", -- TeamSpeak support channel name displayed on blocking screen
		TSDownload = "discord.gg/brsrp", -- Download link displayed on blocking screen
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = escape("[BRS]"); -- Set the local name
		end);
